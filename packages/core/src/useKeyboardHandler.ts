...
export function useKeyboardHandler(deps: KeyboardHandlerDeps) {
  ...
  function handleKeydown(event: KeyboardEvent, blockId: string, _blockIndex: number) {
      ...
      for (const handler of commandHandlers) {
        if (handler.match(beforeCursor)) {
          ...
          handler.execute({
            ...
            blockId,
            textarea,
            updateBlock: (id, newContent) => updateBlock(id, newContent + afterCursor),
            adjustTextareaHeight,
            blockRefs,
          });
          return;
    ...
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      const currentLineEndPos =
        currentLineIndex < lines.length - 1 ? lineStartPos + currentLineLen : content.length;
      const afterCurrentLine =
        currentLineIndex < lines.length - 1 ? content.slice(currentLineEndPos) : '';
